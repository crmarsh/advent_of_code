...
class Slotted(Base):
    def __init__(self, *args, **kwargs):
        try:
            ...
            raise TypeError("%s does not define __slots__" % type(self).__name__)
    ...
    def __str__(self):
        return "%s(%s)" % (
            type(self).__name__,
            ", ".join("%s=%s" % (slot, getattr(self, slot)) for slot in self.__slots__),
        )
...
class World(object):
    ...
    boss_spells = [SpellDesc("Attack", 0, 9, 0, None)]
    ...
    def __str__(self):
        state = [
            ...
            str.format(
                ...
                self.player_state.armor,
                self.player_state.mana,
            ),
